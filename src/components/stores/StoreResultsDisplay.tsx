...
const StoreResultsDisplay: React.FC<StoreResultsDisplayProps> = ({
  ...
  if (error) {
    return <div className="flex items-center justify-center py-12 flex-col">
        <div className="text-center">
          ...
          <Button className="bg-airsoft-red hover:bg-red-700" onClick={handleRetry}>
            Réessayer
          </Button>
        </div>
      </div>;
  ...
  return <div className="space-y-8">
      ...
      {filteredStores.length === 0 ? <div className="text-center py-12">
          ...
          const storeImages = [store.image, store.picture2, store.picture3, store.picture4, store.picture5].filter(Boolean);
          return <Card key={store.id} className="overflow-hidden hover:shadow-lg transition-shadow">
                  <div className="relative">
                    <GameImageCarousel images={storeImages} title={store.name} />
                    <div className="absolute top-2 right-2 flex gap-2">
                      ...
                      {isAdmin && <StoreAdminActions store={store} onStoreUpdate={handleStoreUpdate} />}
                    </div>
                  </div>
                  ...
                  <CardContent className="space-y-2">
                    {store.phone && <div className="flex items-center gap-2 text-sm text-gray-600">
                        <Phone className="h-4 w-4" />
                        {store.phone}
                      </div>}
                    {store.email && <div className="flex items-center gap-2 text-sm text-gray-600">
                        <Mail className="h-4 w-4" />
                        <button onClick={() => handleEmailClick(store.email)} className="text-blue-600 hover:underline cursor-pointer text-left">
                          {store.email}
                        </button>
                      </div>}
                    {store.website && <div className="flex items-center gap-2 text-sm text-gray-600">
