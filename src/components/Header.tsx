      ...
      <div className="max-w-7xl mx-auto flex justify-between items-center py-4">
        ...
        <nav className="hidden md:flex items-center gap-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <AuthSection notificationCount={notificationCount} handleSheetOpenChange={handleSheetOpenChange} />
          </div>
        </nav>

        <Button variant="ghost" size="icon" onClick={() => setIsMenuOpen(!isMenuOpen)} className="md:hidden text-white mx-[10px]">
          {isMenuOpen ? <X size={24} /> : <Menu size={24} />}
        </Button>
      </div>

      <MobileMenu isOpen={isMenuOpen} notificationCount={notificationCount} handleSheetOpenChange={handleSheetOpenChange} isAuthenticated={isAuthenticated} />
    </header>
  );
};
