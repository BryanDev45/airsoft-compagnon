...
const PersonalInfoFields: React.FC<PersonalInfoFieldsProps> = ({
  ...
  return <div className="space-y-4">
      ...
      <ProfileEditableField icon={<Globe className="h-5 w-5" />} label="Langue parlée" value={currentSpokenLanguage} placeholder="Votre langue parlée" isEditing={isEditingLanguage} onEdit={() => setIsEditingLanguage(true)} onSave={handleLanguageSave} onCancel={() => setIsEditingLanguage(false)} isOwnProfile={isOwnProfile} />

      {isOwnProfile && <ProfileEditableField icon={<Phone className="h-5 w-5" />} label="Numéro de téléphone" value={currentPhoneNumber} placeholder="Votre numéro de téléphone" isEditing={isEditingPhone} onEdit={() => setIsEditingPhone(true)} onSave={handlePhoneSave} onCancel={() => setIsEditingPhone(false)} isOwnProfile={isOwnProfile} inputType="tel" />}
    </div>;
};
