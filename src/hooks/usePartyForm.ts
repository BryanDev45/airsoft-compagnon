  ...
  const onSubmit = async (data: PartyFormValues) => {
    ...
    try {
      ...
      const gameData: any = {
        ...
        latitude: data.latitude ? (typeof data.latitude === 'string' ? parseFloat(data.latitude) : data.latitude) : null,
        longitude: data.longitude ? (typeof data.longitude === 'string' ? parseFloat(data.longitude) : data.longitude) : null,
