...
-- Modifier la fonction trigger pour gérer aussi les rejets avec raison
...
BEGIN
  ...
  IF OLD.status != 'rejected' AND NEW.status = 'rejected' THEN
    ...
    -- Créer la notification de rejet avec la raison
    INSERT INTO public.notifications (user_id, title, message, type, related_id)
    VALUES (
      NEW.user_id,
      ...
      CASE 
        ...
        THEN 'Votre demande de vérification a été refusée. Raison : ' || NEW.admin_notes
        ELSE 'Votre demande de vérification a été refusée. Veuillez contacter un administrateur pour plus d''informations.'
      END,
